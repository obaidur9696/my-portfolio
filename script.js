...
function initializePortfolioModal() {
    const modalLinks = document.querySelectorAll('.portfolio-link[data-project]');
    const modal = document.querySelector('.portfolio-modal');
    const modalContent = modal?.querySelector('.modal-content');
    const modalClose = modal?.querySelector('.modal-close');
    const projectData = document.querySelector('.project-data');
    
    if (!modalLinks.length || !modal || !modalContent || !projectData) return;
    ...
    modalLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            const projectDetails = projectData.querySelector(`#${projectId}`)?.innerHTML;
